...
import type { ShWvData } from '@sheep-family/types'
...
export class ShuttleAdapter {
  ...
  static exportToJsonl(data: ShWvData, outputPath: string): string {
    ...
    const content = shuttle.manager.getJsonlContent(data.body.units)
  ...
  static chunkJsonl(data: ShWvData, maxCharsPerLine: number): string {
    const shuttle = new SheepShuttle()
    return shuttle.manager.chunkJsonl(data, maxCharsPerLine)
  }
  ...
  static updateFromJsonl(data: ShWvData, jsonlPath: string): void {
    ...
    const shuttle = new SheepShuttle()
    const updatedUnits = shuttle.manager.updateFromJsonl(data, jsonlContent)
