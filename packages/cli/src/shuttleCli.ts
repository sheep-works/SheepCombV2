...
function loadEnvConfig() {
  ...
  if (fs.existsSync(envPath)) {
    ...
    for (const line of lines) {
      if (line.trim().startsWith('NUXT_PUBLIC_API_KEY=')) {
        config.apiKey = line.split('=')[1].trim().replace(/^["']|["']$/g, '')
      }
      if (line.trim().startsWith('NUXT_PUBLIC_API_BASE_URL=')) {
        config.baseUrl = line.split('=')[1].trim().replace(/^["']|["']$/g, '')
      }
    }
  }
  return config
...
export async function runShuttleMenu(rl: Interface): Promise<void> {
  ...
  if (choice === '13') {
    ...
    const providerChoice = await rl.question('  プロバイダー選択 (1: Local [default], 2: Cloud): ')
    const provider = providerChoice === '2' ? 'honox-cloud' : 'honox-local'
    
    console.log(`  ⌛ 接続確認中 (${provider})...`)
    const shuttle = new CoreShuttle({
      provider: provider,
      honoxUrl: 'http://localhost:8000',
      honoxCloudUrl: config.baseUrl,
      honoxApiKey: config.apiKey,
      honoxCloudApiKey: config.apiKey
    })

    try {
      const greet = await shuttle.requests.greet()
      console.log(`  ✅ 接続成功！`)
      console.log(`  ステータス: ${greet.status}`)
      console.log(`  モデル情報: ${greet.model_info}`)
    } catch (e: any) {
  ...
  if (choice === '14' || choice === '15') {
    ...
    const customPrompt = await rl.question('  カスタムプロンプト (任意、空欄でデフォルト): ')
    ...
    shuttle.createChunks('data', 4000)

    if (shuttle.chunks.length === 0) {
      console.log('  ⚠ 処理対象のデータチャンクがありません。')
      return
    }

    console.log(`  🚀 処理開始 (チャンク数: ${shuttle.chunks.length})`)
    const target = choice === '14' ? 'TRANSLATE' : 'CHECK'

    try {
      for (let i = 0; i < shuttle.chunks.length; i++) {
        console.log(`  ⌛ チャンク ${i + 1}/${shuttle.chunks.length} を処理中...`)
        await shuttle.processRequests(i, target, customPrompt || undefined)
        
        const chunk = shuttle.chunks[i]!
        if (chunk.status === 'error') {
          throw new Error(`チャンク ${i} でエラーが発生しました: ${chunk.response}`)
        }
      }

      // Merge the results back
      console.log('  ⌛ データをマージ中...')
      let combinedJsonl = ''
      for (const chunk of shuttle.chunks) {
        combinedJsonl += chunk.response + '\n'
      }
      
      const updatedUnits = shuttle.manager.updateFromJsonl(data, combinedJsonl)
      data.body.units = updatedUnits

      const outName = choice === '14' ? 'translated.json' : 'checked.json'
      const outPath = path.join(PATHS.outDir, outName)
      ensureOutDir()
      fs.writeFileSync(outPath, JSON.stringify(data, null, 2), 'utf-8')
      console.log(`  💾 処理完了！結果を保存しました: ${outPath}`)

    } catch (e: any) {
      console.log(`  ❌ エラーにより処理を中断しました: ${e.message}`)
    }
    return
  }
