...
export class RequestHonoxLocal implements SubLlmProvider {
    ...
    constructor(parent: SheepShuttle, options?: ShuttleOptions) {
        this.parent = parent
        this.API_KEY = options?.honoxApiKey || ''
        this.BASE_URL = 'http://localhost:8000'
        this.updateBaseUrl(options)
    }

    public updateOptions(options: Partial<ShuttleOptions> = {}) {
        if (options.honoxApiKey !== undefined) this.API_KEY = options.honoxApiKey
        this.updateBaseUrl(options)
    }

    private updateBaseUrl(options?: Partial<ShuttleOptions>) {
        this.BASE_URL = (options?.honoxUrl || 'http://localhost:8000').replace(/\/$/, '')
    }
