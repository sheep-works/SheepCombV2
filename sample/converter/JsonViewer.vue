...
const dropLabel = computed(() => {
  if (mode.value === 'raw') return 'JSON 繝輔ぃ繧､繝ｫ繧偵ラ繝ｭ繝・・縺ｾ縺溘・繧ｯ繝ｪ繝・け縺励※驕ｸ謚・
  return 'JSONL 繝輔ぃ繧､繝ｫ繧偵ラ繝ｭ繝・・縺ｾ縺溘・繧ｯ繝ｪ繝・け縺励※驕ｸ謚・
})
...
<template>
  <div class="jc-container">
    ...
    >
      ...
      <div v-else>
        <p class="jc-loaded-label">笨・File Loaded</p>
    ...
    <div v-if="mode === 'pinia' && !piniaHasData" class="jc-paste-area">
      <textarea
        v-model="piniaInput"
        class="jc-textarea"
        placeholder="Pinia 繧ｹ繝医い縺ｮ JSON 譁・ｭ怜・繧偵％縺薙↓雋ｼ繧贋ｻ倥￠窶ｦ"
        rows="6"
      ></textarea>
      <p v-if="piniaError" class="jc-parse-error">笞 {{ piniaError }}</p>
      <button @click="parsePiniaState" class="jc-btn jc-btn-parse" :disabled="!piniaInput.trim()">隗｣譫・/button>
    </div>

    <!-- ===================== MODE 1: Chunked JSONL ===================== -->
    <template v-if="mode === 'chunked' && chunks.length">
      <div class="jc-controls">
        <button @click="crtChunkIdx--" :disabled="crtChunkIdx <= 0" class="jc-btn">竊・Previous</button>
        ...
        <button @click="crtChunkIdx++" :disabled="crtChunkIdx >= chunks.length - 1" class="jc-btn">Next 竊・/button>
    ...
    <template v-if="mode === 'jsonl' && jsonlItems.length">
      <div class="jc-controls">
        <button @click="jsonlPage--" :disabled="jsonlPage <= 0" class="jc-btn">竊・Previous</button>
        <div class="jc-chunk-info">
          Page {{ jsonlPage + 1 }} / {{ jsonlTotalPages }}
          <span class="jc-chunk-chars">({{ jsonlItems.length }} items)</span>
        </div>
        <button @click="jsonlPage++" :disabled="jsonlPage >= jsonlTotalPages - 1" class="jc-btn">Next 竊・/button>
        <button @click="resetAll" class="jc-btn jc-btn-danger">Clear</button>
      </div>
    ...
    <template v-if="mode === 'raw' && rawUnits.length">
      ...
      <div v-if="rawMeta" class="jc-meta-panel">
        ...
        <div class="jc-meta-grid">
          <div class="jc-meta-row" v-if="rawMeta.sourceLang || rawMeta.targetLang">
            ...
            <span class="jc-meta-val">{{ rawMeta.sourceLang || '(auto)' }} 竊・{{ rawMeta.targetLang || '(auto)' }}</span>
          </div>
          <div class="jc-meta-row">
            ...
            <div class="jc-meta-files">
              <div v-for="(f, fi) in rawMeta.files" :key="fi" class="jc-meta-file">
                ...
                <span class="jc-meta-file-range">#{{ f.start }}窶怒{ f.end }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Controls -->
      <div class="jc-controls">
        <button @click="rawPage--" :disabled="rawPage <= 0" class="jc-btn">竊・Previous</button>
        <div class="jc-chunk-info">
          Page {{ rawPage + 1 }} / {{ rawTotalPages }}
          <span class="jc-chunk-chars">({{ rawUnits.length }} units)</span>
        </div>
        <button @click="rawPage++" :disabled="rawPage >= rawTotalPages - 1" class="jc-btn">Next 竊・/button>
        <button @click="resetAll" class="jc-btn jc-btn-danger">Clear</button>
      </div>
    ...
    <template v-if="mode === 'pinia' && piniaHasData">
      ...
      <div class="jc-meta-panel">
        ...
        <div class="jc-meta-grid">
          ...
          <div class="jc-meta-row" v-if="piniaMeta">
            <span class="jc-meta-key">Languages</span>
            <span class="jc-meta-val">{{ piniaMeta.sourceLang || '(auto)' }} 竊・{{ piniaMeta.targetLang || '(auto)' }}</span>
          </div>
          <div class="jc-meta-row" v-if="piniaMeta">
      ...
      <!-- Controls -->
      <div class="jc-controls">
        <button @click="piniaPage--" :disabled="piniaPage <= 0" class="jc-btn">竊・Previous</button>
        <div class="jc-chunk-info">
          Page {{ piniaPage + 1 }} / {{ piniaTotalPages }}
          <span class="jc-chunk-chars">({{ piniaUnits.length }} units)</span>
        </div>
        <button @click="piniaPage++" :disabled="piniaPage >= piniaTotalPages - 1" class="jc-btn">Next 竊・/button>
      ...
      <table class="jc-data-table">
        ...
        <tbody v-for="unit in piniaPageItems" :key="unit.idx" class="jc-chunk-tbody"
          ...
          <tr class="jc-chunk-row">
            <td class="jc-td-id">
              {{ unit.idx }}
              <span v-if="unit.idx === piniaCrtPos" class="jc-pos-badge">笆ｶ</span>
            </td>
            <td class="jc-content-row">
              <div class="jc-side-box">
                <span class="jc-label jc-label-src">SOURCE</span>
