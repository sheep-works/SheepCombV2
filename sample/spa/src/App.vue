...
onMounted(async () => {
  try {
    statusMsg.value = { text: 'WASM Runtime 繧貞・譛溷喧荳ｭ...', type: 'info' }
    await initWasm()
    isWasmReady.value = true
    statusMsg.value = { text: '貅門ｙ螳御ｺ・, type: 'success' }
  } catch (e) {
    statusMsg.value = { text: 'WASM 縺ｮ蛻晄悄蛹悶↓螟ｱ謨励＠縺ｾ縺励◆', type: 'error' }
  }
})
...
const execParse = async () => {
  ...
  statusMsg.value = { text: '隗｣譫蝉ｸｭ...', type: 'info' }
  ...
  try {
    ...
    segments.value = all
    statusMsg.value = { text: `謚ｽ蜃ｺ螳御ｺ・ ${all.length} 莉ｶ`, type: 'success' }
  } catch (e: any) {
    statusMsg.value = { text: `繧ｨ繝ｩ繝ｼ: ${e.message}`, type: 'error' }
  } finally {
...
const execConsistency = async () => {
  ...
  isLoading.value = true
  statusMsg.value = { text: '繧・ｌ繝√ぉ繝・け險育ｮ嶺ｸｭ (WASM)...', type: 'info' }
  
  try {
    const wasm = getWasm()
    // 蟶ｸ縺ｫ迴ｾ蝨ｨ縺ｮ segments.value 繧貞ｯｾ雎｡縺ｫ縺吶ｋ
    ...
    segments.value = flat
    statusMsg.value = { text: `繧・ｌ繝√ぉ繝・け螳御ｺ・ ${groups.length} 繧ｰ繝ｫ繝ｼ繝輿, type: 'success' }
  } catch (e: any) {
    statusMsg.value = { text: `險育ｮ励お繝ｩ繝ｼ: ${e.message}`, type: 'error' }
  } finally {
    isLoading.value = false
  }
}
        ...
        <div class="card">
          ...
          <div class="upload-section">
            ...
            >
              ...
              <p v-if="srcFiles.length === 0">蟇ｾ雎｡繝輔ぃ繧､繝ｫ繧偵ラ繝ｭ繝・・</p>
              <p v-else class="file-count">{{ srcFiles.length }} 蛟矩∈謚樔ｸｭ</p>
            </div>
      ...
      <section class="results-area">
        <div class="card full-height">
          ...
          <div class="empty-state" v-else>
            ...
            <p>繝輔ぃ繧､繝ｫ繧偵い繝・・繝ｭ繝ｼ繝峨＠縺ｦ隗｣譫舌ｒ髢句ｧ九＠縺ｦ縺上□縺輔＞</p>
          </div>
        </div>
      </section>
    </main>
