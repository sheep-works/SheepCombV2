...
export interface Segment {
  ...
  tgt: string;
  note?: string;
  [key: string]: any; // for similarity results etc.
}
...
export async function parseDocx(content: ArrayBuffer): Promise<Segment[]> {
  ...
  try {
    // 1. memoQ Detection
    if (numTables === 1 && firstTableText.includes("驥崎ｦ・ｼ√そ繧ｰ繝｡繝ｳ繝・D繧・た繝ｼ繧ｹ繝・く繧ｹ繝医ｒ螟画峩縺励↑縺・〒縺上□縺輔＞")) {
      return extractMemoQ(tables[0]);
    }
    
    // 2. Xbench Detection
    // Xbench checks tables[0]._element.xml. In DOM, we check the outerHTML of the table node.
